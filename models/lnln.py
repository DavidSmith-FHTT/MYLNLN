...
from models.CrossMamba import CrossMambaBlock
...
class LNLN(nn.Module):
    ...
    def forward(self, complete_input, incomplete_input):
        ...
        b = vision_m.size(0)  # batch size 64

        h_1_v = self.proj_v(vision_m)[:, :8]  # dim: [64, 8, 128]
        h_1_a = self.proj_a(audio_m)[:, :8]  # [64, 8, 128]
        h_1_l = self.proj_l(self.bertmodel(language_m))[:, :8]  # [64, 8, 128]
        ...
        if (vision is not None) and (audio is not None) and (language is not None):
            ...
            complete_feats = torch.cat([complete_audio_feat, complete_vision_feat, complete_language_feat],
                                       dim=1)  # as the label of reconstruction
